...
<body>
    ...
    if (isset($_SESSION["email"]) && $_SESSION["role"] == "prof") {
        ...
        if ($_POST) {
            ...
            $sql = "SELECT * FROM cursos WHERE Actiu like 'si' and DNI_prof like '$dni' and Data_final>'$fechaActual' and Nom LIKE '%$nom%'";
            ...
            // Creamos la sentencia sql para cursos que han acabado
            $sqlCad = "SELECT * FROM cursos WHERE Actiu like 'si'  and Data_final<'$fechaActual' and DNI_prof like '$dni' and Nom LIKE '%$nom%'";
            // Ejecutamos la sentencia
            $consultaCad = mysqli_query($bddcon, $sqlCad);
            // Generamos la lista con en resultado de la consulta
            $numlinesCad = mysqli_num_rows($consultaCad);
            if ($numlinesCad == 0) {
            } else {
                echo "<div class='caducadosDivisor'>Cursos que han acabat</div>";
                for ($i = 0; $i < $numlinesCad; $i++) {
                    echo "<table class='cursTable'>";
                    $curs = mysqli_fetch_assoc($consultaCad);
                    $codiCurs = $curs['Codi'];
                    $nomCurs = $curs['Nom'];
                    $sql2 = "SELECT * FROM matricula WHERE Codi_curs like '$codiCurs'";
                    $consulta2 = mysqli_query($bddcon, $sql2);
            ...
            // Creamos la sentencia sql para cursos que han acabado
            $sqlCad = "SELECT * FROM cursos WHERE Actiu like 'si'  and Data_final<'$fechaActual' and DNI_prof like '$dni'";
            // Ejecutamos la sentencia
            $consultaCad = mysqli_query($bddcon, $sqlCad);
            // Generamos la lista con en resultado de la consulta
            $numlinesCad = mysqli_num_rows($consultaCad);
